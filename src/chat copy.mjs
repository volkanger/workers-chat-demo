...
export default {
  async fetch(request, env) {
    return await handleErrors(request, async () => {
      ...
      switch (path[0]) { // if not root, but have more; let's see if it's api
        case "api":
...
async function handleApiRequest(path, request, env) {
  ...
  //path0 is the directories, like /api/whatever here-- they made it really shitty
  ...
  async callLimiter() {
    try {
      ...
      try {
        ...
        response = await this.limiter.fetch("https://limiter-checking-things-dummy-url", {method: "POST"});
      } catch (err) {
